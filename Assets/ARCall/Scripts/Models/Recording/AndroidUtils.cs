...
public class AndroidUtils : MonoBehaviour
{
    ...
    public static void ShareAndroid(string body, string subject, string url, string filePath, string mimeType, bool chooser, string chooserText)
    {
        ...
        using (AndroidJavaObject intentObject = new AndroidJavaObject("android.content.Intent"))
        {
            ...
            { }
            using (intentObject.Call<AndroidJavaObject>("setFlags", intentClass.GetStatic<int>("FLAG_GRANT_READ_URI_PERMISSION")))
            { }
            ...
            else if (filePath != null)
            {
                ...
                AndroidJavaObject uri = fileProvider.CallStatic<AndroidJavaObject>("getUriForFile", currentActivity, authority, fileObj);

                // using (AndroidJavaClass uriClass = new AndroidJavaClass("android.net.Uri"))
                // using (AndroidJavaObject uriObject = uriClass.CallStatic<AndroidJavaObject>("parse", "content://" + filePath))
                using (intentObject.Call<AndroidJavaObject>("setDataAndType",uri, "video/mp4"))
                { }
            }
