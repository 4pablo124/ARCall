...
/**
 * @author zeh fernando / edited by Diego Santos Leão to allow change text color and to hide the navigation bar
 */
class ApplicationChrome
{
	...
	private static StatusBarTextColor _statusBarTextColor;
	...
	private static bool _isStatusBarCompletelyHidden;
		...
		private static void applyUIStatesAndroid() {
			...
			switch (_statusBarState) {
				...
				case States.TranslucentOverContent:
					...
					break;
				case States.Hidden:
					_isStatusBarCompletelyHidden = true;
			...
			switch (_navigationBarState) {
				...
				case States.Hidden:
					newSystemUiVisibilityValue |= VIEW_SYSTEM_UI_FLAG_LAYOUT_STABLE | VIEW_SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION 
	                                           | VIEW_SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | VIEW_SYSTEM_UI_FLAG_HIDE_NAVIGATION
	                                           | VIEW_SYSTEM_UI_FLAG_FULLSCREEN | VIEW_SYSTEM_UI_FLAG_IMMERSIVE_STICKY;
	   			    break;
			}
	
			// Apply status bar COLOR (light or default dark)
			switch (_statusBarTextColor) {
				case StatusBarTextColor.Default:
					break;
				case StatusBarTextColor.LightStatusBar:
					newSystemUiVisibilityValue |= SYSTEM_UI_FLAG_LIGHT_STATUS_BAR;
					break;
			}

			//if (Screen.fullScreen) Screen.fullScreen = false; //REMOVED, because doing this makes the Navigation Bar always appear
	...
	public static StatusBarTextColor statusBarTextColor
	{
		get { return _statusBarTextColor; }
		set
		{
			if (_statusBarTextColor != value)
			{
				_statusBarTextColor = value;
				applyUIStates();
			}
		}
	}
